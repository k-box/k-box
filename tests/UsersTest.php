...
class UsersTest extends DbTestCase {
	...
	public function testUserSeeding()
	{
		...
		$this->assertTrue($user->can(Capability::UPLOAD_DOCUMENTS));
		
		// test has all capabilities
		$this->assertTrue($user->canAll(Capability::$ADMIN));
		
		// test remove capability (1 or more)
		
		$user->removeCapability(Capability::UPLOAD_DOCUMENTS);
		
		$this->assertFalse($user->can(Capability::UPLOAD_DOCUMENTS), 'Removed UPLOAD_DOCUMENTS capability');
		$this->assertFalse($user->canAll(Capability::$ADMIN), 'Test all ADMIN capavbilities after removing UPLOAD_DOCUMENTS');
		
		$user->addCapability(Capability::UPLOAD_DOCUMENTS);
		
		$this->assertTrue($user->can(Capability::UPLOAD_DOCUMENTS), 'Adding back a capability');
