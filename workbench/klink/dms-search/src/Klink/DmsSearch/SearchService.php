...
class SearchService {
	 ...
	 */
	public function search( $terms, $offset = 0, $resultsPerPage = 10, $visibility = 'public', array $facets = null)
	{
		try{
	 ...
	 */
	public function getTotalIndexedDocuments($visibility = 'public')
	{		
...
//			$default_facets = \KlinkFacetsBuilder::create()->institution()->documentType()->language()->build(); // usefull for the elastic list, so we setup always that
			...
			$test = $this->search( $search_terms, $limit * ($page - 1), $limit, $visibility, $facets_to_apply );
...
//var_dump($test->getFacets());
	...
	/*
	 * get the search parameters contained in the request 
	 */
...
//		}
	...
	public function defaultFacets($visibility='private'){
		...
		return \Cache::remember('dms_default_facets_'.$visibility, 200, function() use($visibility) {
			...
			return $this->adapter->getConnection()->facets($default_array, $visibility);
		});
	}
		...
		if(!empty($ints)){
			...
			foreach($filters as $filter){
				if(isset($filter['options'])){
		            ...
		            $vals = str_replace($filter['field'].':', '', $filter['query']);
...
//			dd(compact('filters', 'hacked', 'ints'));
			return $hacked;
		}
