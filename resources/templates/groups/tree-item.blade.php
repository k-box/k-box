...
<li class="tree-item groups-menu" data-group-id="{{$group->id}}" data-isprivate="{{$group->is_private ? 'true':'false'}}">
	...
	<a href="{{ DmsRouting::group($group->id) }}" title="{{$group->name}}" class="tree-item-inner @if(\Request::is('*groups/'. $group->id)) current @endif" data-group-id="{{$group->id}}" data-drop="true" draggable="true" data-drag-el="group" data-drop-action="copyTo" @if( isset($badge_shared) && $badge_shared ) data-shared="true"  @endif data-isprivate="{{$group->is_private ? 'true':'false'}}">
