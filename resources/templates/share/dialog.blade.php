<div class="dialog--share js-sharing">
	...
	<div class="dialog__inner">
		...
		@if( !is_null( $sharing_links ) && !empty($sharing_links) && !$is_multiple_selection )
			...
			<div class="copy-links dialog__section js-link-section">
				...
				<div class="dialog__section__inner copy-link ">
					...
					<a class="button copy-link__button copy-link__button--mail" title="{{ trans($elements_count == 1 ? 'share.send_link' : 'share.send_link_multiple') }}" href="mailto:?body={{ urlencode($sharing_links) }}">
						<span class="btn-icon icon-content-black icon-content-black-ic_mail_black_24dp"></span>
					</a>
		...
		<div class="dialog__section js-share-section">
			<h6 class="dialog__section__title">{{ trans('share.dialog.section_access_title') }}</h6>
					@unless($is_multiple_selection)
						...
						<select name="linktype" id="linktype" class="js-link-type c-form__input--full-width">
		...
		@endif
			...
			<div class="dialog__section--add">
				...
				<select class="js-select-users" name="users[]" id="users" multiple="multiple" style="min-width:auto !important">
				...
				@if($elements_count == 1 && !$is_collection)
					...
					<div class="c-switch js-publish-switch" data-is-public="{{ $is_public ? 'true' : 'false' }}" data-network="{{network_name()}}">
						...
						<div class="c-switch__label js-publish-switch-label">
							{{ trans( $is_public ? 'share.dialog.published' : 'share.dialog.not_published', ['network' => network_name()]) }}
						</div>

						<div class="c-switch__buttons">
							@unless(!$can_make_public && $is_public)
							<button @if(!$can_make_public) disabled @endif class="c-switch__button js-publish-switch-button @unless($is_public) c-switch__button--selected @endif" data-action="make_private" title="{{ trans('actions.make_private') }}">
								<svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24"><path d="M18 8h-1V6c0-2.76-2.24-5-5-5S7 3.24 7 6v2H6c-1.1 0-2 .9-2 2v10c0 1.1.9 2 2 2h12c1.1 0 2-.9 2-2V10c0-1.1-.9-2-2-2zm-6 9c-1.1 0-2-.9-2-2s.9-2 2-2 2 .9 2 2-.9 2-2 2zm3.1-9H8.9V6c0-1.71 1.39-3.1 3.1-3.1 1.71 0 3.1 1.39 3.1 3.1v2z"/></svg>
							</button>
							@endif

							@unless(!$can_make_public && !$is_public)
							<button @if(!$can_make_public) disabled @endif class="c-switch__button js-publish-switch-button @if($is_public) c-switch__button--selected @endif" data-action="make_public" title="{{ trans('networks.publish_to_short') }}">
	...
	<div class="dialog__buttons">
		<button class="cancel js-cancel">{{ trans('actions.done') }}</button>
	</div>
