@if(isset($is_in_collection) && $is_in_collection && isset($collections))
    ...
    @foreach($collections as $group)
        ...
        <div class="badge" data-group-id="{{$group->id}}" data-document-id="{{$document_id}}" @if($group->color) style="background-color:#{{$group->color}}" @endif>
            <a href="{{route( $use_groups_page ? 'documents.groups.show' : 'shares.group' , [ 'id' => $group->id, 'highlight' => $document_id])}}" class="badge-link hint--bottom" data-hint="{{ trans('panels.collection_open', ['collection' => $group->name ])}}"> 
                {{$group->getAncestors()->sortByDesc('depth')->implode('name', ' > ')}}
            </a>
