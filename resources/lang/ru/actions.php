<?php

return [

    /*
    |--------------------------------------------------------------------------
    | Actions Language Lines
    |--------------------------------------------------------------------------
    |
    | The following language lines are used primarly for buttons and switcher
    |
    */

    'edit' => 'Изменить',
    'details' => 'Детали',
    'expand'     => 'Расширить',
    'collapse'     => 'Свернуть',
    'expand_all'     => 'Расширить все',
    'collapse_all'     => 'Свернуть все',
    'open' => 'Открыть',

    'restore' => 'Восстановить',
    'disable' => 'Отключить',

    'cancel' => 'Отменить',
    'done' => 'Готово',

    'or_alt' => 'или',
    'or' => 'или :action',

    'selection' => [

        'dropdown' => 'Выбор',
        'all' => 'Все',
        'clear' => 'Очистить',
        'invert' => 'Инвертировать',
        'hint' => 'Выбрать все/Очистить выбранное',
     
        'at_least_one_document' => 'Выберите хотя бы один документ',
        'at_least_one' => 'Выберите хотя бы один документ или коллекцию',
        'only_one' => 'Выберите только один документ или коллекцию',
    ],
 
    'clipboard' => [
        'copied_title' => 'Скопировано',
        'copied_link_text' => 'Ссылка была скопирована',
        
        'not_copied_title' => 'Невозможно скопировать',
        'not_copied_link_text' => 'Ссылка не может быть скопирована. Вы можете скопировать ее вручную, с помощью комбинации Ctrl+C на клавиатуре.',
    ],

    'switcher' => [

        'details' => 'Детали',
        'tiles' => 'Плитка',
        'grid' => 'Сетка',
        'map' => 'Карта',

    ],

    'versions' => [

        'manage' => 'Управлять версиями',
        'add_new' => 'Добавить новую версию',

    ],

    'upload' => 'Загрузить документ',
    'upload_video' => 'Загрузить видео',
    'upload_alt' => 'Загрузить',

    'save' => 'Сохранить',
    'saving' => 'Сохранение...',
    'deleting' => 'Удаление...',
    'restoring' => 'Восстановление...',
    'cleaning_trash' => 'Удаление из корзины...',
    'cleaning_trash_wait' => 'Пожалуйста, подождите пока совершается очистка корзины...',

    'add_to' => 'Добавить к',
    'move_to' => 'Переместить в',

    'add_or_move_to' => 'Коллекции',

    'publish_documents' => 'Опубликовать',
    'make_private' => 'Сохранить закрытым',

    'create_add_dropdown' => 'Создать или добавить',

    'trash_btn' => 'В корзину',
    'trash_btn_alt' => 'В корзину',
    'forcedelete_btn_alt' => 'Удалить безвозвратно',
    'restore_btn' => 'Восстановить',
    'empty_trash' => 'Очистить корзину',

    'create_btn' => 'Создать',

    'create_collection_btn' => 'Создать коллекцию',
    
    'filters' => [
        'filter' => 'Фильтры',
        'clear_filters' => 'Очистить фильтры',
        'collection_locked' => 'Данная коллекция просматривается Вами и не может быть удалена.'
    ],
    
    'hints' => [
        'make_public' => 'Выберите несколько документов, прежде чем предоставить им открытый доступ в Сети K-Link',
    ],

    'not_available' => 'Данное действие недоступно',

    // buttons on the dialog
    'dialogs' => [
        'cancel_btn' => 'Нет',
        'cancel_btn_alt' => 'Отменить',
        
        'yes_btn' => 'Да',
        'no_btn' => 'Нет',
        'ok_btn' => 'OK',
        
        'delete_btn' => 'Удалить',
        'remove_btn' => 'Да',
        'trash_btn' => 'Да',
        'move_btn' => 'Да',
        
        // showed when on a dialog when the input field is empty, but a value is required
        'input_required' => 'Напишите что-нибудь',
        
        
    ],
    
    'got_it' => 'Согласен',
    
    'remove' => 'Удалить',
    
    'sort_by' => [
        'label' => 'Сортировать по',
        'oldest_first' => 'Самые ранние',
        'newest_first' => 'Недавние',
    ],

];
